...
def loggedIn(func):
    def checkLogin(*args, **kwargs):
        if args[0].can_use_timeline:
            ...
            raise Exception("Timelineを使用できません")
...
class Timeline:
    ...
    def getTimelintTab(self, postLimit=20, likeLimit=6, commentLimit=10, requestTime=0):
        ...
        hr = self.server.additionalHeaders(
            self.server.timelineHeaders,
            {"x-lhm": "POST", "Content-type": "application/json", "x-lsr": "JP"},
        )
    ...
    @loggedIn
    def getSquarePost(self, pid: str, postId: str, sourceType: str = "SQUARE"):
        params = {"homeId": pid, "postId": postId, "sourceType": sourceType}
        hr = self.server.additionalHeaders(
            self.server.timelineHeaders,
            {
                "x-lhm": "GET",
        ...
        url = self.server.urlEncode(
            "https://legy-jp.line-apps.com", "/sn/api/v57/post/delete.json", params
        )
        r = self.server.postContent(url, headers=hr)
        return r.json()

    @loggedIn
    def deleteSquarePost(self, homeId: str, postId: str):
    ...
    def sendPostToTalk(self, postId: str, receiveMids: list):
        ...
        url = self.server.urlEncode(
            self.LINE_HOST_DOMAIN, "/sn/api/v57/post/sendPostToTalk.json", {}
        )
        hr = self.server.additionalHeaders(
            self.server.timelineHeaders,
            {
    ...
    @loggedIn
    def translate(self, originalText: str, sLang: str = "auto", tLang: str = "JP"):
        data = {
